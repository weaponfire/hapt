...
extern __shared__ uint_64 sarray[]; 

#ifdef HASATOMICS
__device__ uint_64 ohtotal = 0;
#endif

#define min(x,y) ((x<y)?x:y)
#define max(x,y) ((x>y)?x:y)

#define swap(a,b) uint_64 _tmp=a; a=b; b=_tmp;
...
__device__
inline void swap(uint_64& a, uint_64& b)
{
	uint_64 tmp = a;
	a = b;
	b = tmp;
...
//template <typename unsigned int>
__device__ inline
void bitonicSort(uint_64* fromvalues, uint_64* tovalues, unsigned int from, unsigned int size)
{
	uint_64* shared = (uint_64*)sarray;
 ...
 */
__device__ inline void cumcount(uint_64 *lblock, uint_64 *rblock)
{
	int tx = threadIdx.x;
 ...
 */
//template <typename unsigned int>
__global__ void part1(uint_64* data, Params* params, Hist* hist, Length* lengths)
{
	const int tx = threadIdx.x;

	uint_64* lblock = (uint_64*)sarray;
	uint_64* rblock = (uint_64*)(&lblock[(blockDim.x+1)]);
	uint_64* minpiv = (uint_64*)(&rblock[(blockDim.x+1)]);
	uint_64* maxpiv = (uint_64*)(&minpiv[blockDim.x]); 


	// Where should we read?
	unsigned int start = params[blockIdx.x].from;
	unsigned int end = params[blockIdx.x].end;
	uint_64 pivot = params[blockIdx.x].pivot;
	...
	cumcount((uint_64*)lblock,(uint_64*)rblock);
 ...
 */
//template <typename unsigned int>
__global__ void part2(uint_64* data, uint_64* data2, Params* params, Hist* hist, Length* lengths)
{
	const int tx = threadIdx.x;
 ...
 */
//template <typename unsigned int>
__global__ void part3(uint_64* data, Params* params, Hist* hist, Length* lengths)
{
	const int tx = threadIdx.x;
	const int bx = blockIdx.x;
 ...
 */
//template <typename unsigned int>
__global__ void lqsort(uint_64* adata, uint_64* adata2, LQSortParams* bs, unsigned int phase)
{
	...
	__shared__ uint_64* data;
	__shared__ uint_64* data2;
	...
	while(bx<gridDim.x)
	{
		...
		while(bi>=0)
		{
			...
			if(tx==0)
			{
				...
				uint_64 mip = min(min(data[from],data[to-1]),data[(from+to)/2]);
				uint_64 map = max(max(data[from],data[to-1]),data[(from+to)/2]);
